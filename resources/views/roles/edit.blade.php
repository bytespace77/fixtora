...
@section('content')
  <div style="max-width: 1040px; margin: 0; padding-bottom: 40px;">

    <h1 style="font-size:24px; font-weight:800; letter-spacing:-.6px; color:var(--navy); SSmargin-bottom:24px;">User Role
      – {{ $role->name }}</h1>

    @if(session('success'))
      <div
        style="background:var(--green-bg); border:1px solid #bbf7d0; color:var(--green); padding:10px 14px; border-radius:var(--radius-sm); font-size:13px; margin-bottom:18px;">
        {{ session('success') }}
      </div>
    @endif
    ...
    @if($errors->any())
      <div
        style="background:var(--red-bg); border:1px solid #fecaca; color:var(--red); padding:10px 14px; border-radius:var(--radius-sm); font-size:13px; margin-bottom:18px;">
    ...
    <div style="display:grid; grid-template-columns:1fr 320px; gap:20px; align-items:start;">
      ...
      <div>
        ...
        {{-- Users in role --}}
        <div
          style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius); padding:24px; margin-bottom:20px; box-shadow:var(--shadow-sm);">
          ...
          @if(auth()->user()->hasPermission('assign_users_to_role'))
            <form action="{{ route('roles.association', $role) }}" method="POST">
              ...
              <div
                style="display:grid; grid-template-columns:1fr 40px 1fr; gap:12px; align-items:start; margin-bottom:14px;">

                {{-- Unassigned --}}
                <div>
                  <label
                    style="display:block; font-size:12px; font-weight:600; color:var(--muted); margin-bottom:6px;">Unassigned
                    users</label>
                ...
                {{-- Arrow --}}
                <div
                  style="display:flex; flex-direction:column; align-items:center; justify-content:center; gap:6px; padding-top:26px;">
                ...
                <div>
                  <label
                    style="display:block; font-size:12px; font-weight:600; color:var(--muted); margin-bottom:6px;">Users
                    assigned to role</label>
        ...
        {{-- Permissions --}}
        <div
          style="background:var(--surface); border:1px solid var(--border); border-radius:var(--radius); padding:24px; box-shadow:var(--shadow-sm);">
          <h2 style="font-size:14px; font-weight:700; margin-bottom:16px; color:var(--text);">Permissions</h2>

          {{-- Tabs --}}
          <div
            style="display:flex; flex-wrap:wrap; gap:4px; margin-bottom:20px; border-bottom:1px solid var(--border); padding-bottom:12px;">
                ...
                <div id="panel-{{ Str::slug($group) }}" class="perm-panel" style="{{ $loop->first ? '' : 'display:none;' }}">
                  ...
                  <div style="display:grid; grid-template-columns:repeat(3, 1fr); gap:8px;">
                    @foreach($perms as $perm)
                      <label
                        style="display:flex; align-items:center; gap:7px; font-size:12px; color:var(--text-2); cursor:pointer;">
                        <input type="checkbox" name="permissions[]" value="{{ $perm }}" {{ in_array($perm, $role->permissions ?? []) ? 'checked' : '' }} class="perm-cb-{{ Str::slug($group) }}"
        ...
        @if(auth()->user()->hasPermission('edit_roles'))
          <form action="{{ route('roles.update', $role) }}" method="POST">
            ...
            <div style="margin-bottom:14px;">
              <label style="display:block; font-size:12px; font-weight:600; color:var(--muted); margin-bottom:5px;">Role
                name</label>
            ...
            <div style="margin-bottom:16px;">
              <label
                style="display:block; font-size:12px; font-weight:600; color:var(--muted); margin-bottom:5px;">Description</label>
              <textarea name="description" rows="4"
                ...
                onblur="this.style.borderColor='var(--border)'">{{ old('description', $role->description) }}</textarea>
            </div>
