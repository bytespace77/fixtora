...
<div class="page-header">
  ...
  <div class="header-actions">
    ...
    <!-- Export -->
    @if(auth()->user()->hasPermission('export_dashboard'))
    <div class="export-wrap">
      ...
      <div class="export-dropdown" id="exportMenu">
        <a class="export-option" href="{{ route('home') }}?range={{ $range }}&from={{ $from->format('Y-m-d') }}&to={{ $to->format('Y-m-d') }}&export=pdf" target="_blank">
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg>
          Export as PDF
        </a>
        <a class="export-option" href="{{ route('home') }}?range={{ $range }}&from={{ $from->format('Y-m-d') }}&to={{ $to->format('Y-m-d') }}&export=excel">
